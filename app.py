...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    @media (max-width: 980px) { .hero, .lower-grid { grid-template-columns: 1fr; } .status-card { min-height: 0; } }
    @media (max-width: 720px) { .shell { display: block; } .sidebar { gap: 18px; padding: 15px 14px; border-right: 0; border-bottom: 1px solid var(--line); } nav { display: flex; overflow-x: auto; } .nav-item { width: auto; white-space: nowrap; } .side-note { display: none; } .content { padding: 24px 16px 42px; } .topbar { margin-bottom: 24px; } .operator span { display: none; } .metrics { grid-template-columns: repeat(2, 1fr); } .settings-grid { grid-template-columns: 1fr; } .hero-card { padding: 23px; } }
  </style>
</head>
<body>
  <div class="shell">
